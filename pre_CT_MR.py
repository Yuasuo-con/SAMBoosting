...
parser.add_argument("-img_name_prefix", type=str, default="_0000.nii.gz",
...
img_name_prefix = args.img_name_prefix  # "_0000.nii.gz"
gt_name_prefix = args.gt_name_prefix  # ".nii.gz"
...
names = [
    name
    for name in names
    if os.path.exists(join(nii_path, img_name_prefix + name.split(gt_name_prefix)[1]))
]
...
def preprocess(name, npz_path):
    ...
    """
    image_name = img_name_prefix + name.split(gt_name_prefix)[1]
    gt_name = name
    ...
    if len(z_index) > 0:
        ...
        np.savez_compressed(join(npz_path, prefix + gt_name.split('.nii.gz')[0]+'.npz'), imgs=img_roi, gts=gt_roi, spacing=img_sitk.GetSpacing())
        ...
        if save_nii:
            ...
            sitk.WriteImage(
                img_roi_sitk,
                join(npz_path, prefix + gt_name.split('.nii.gz')[0] + "_img.nii.gz"),
            )
            gt_roi_sitk = sitk.GetImageFromArray(gt_roi)
            gt_roi_sitk.SetSpacing(img_sitk.GetSpacing())
            sitk.WriteImage(
                gt_roi_sitk,
                join(npz_path, prefix + gt_name.split('.nii.gz')[0] + "_gt.nii.gz"),
            )
